...
autoFIPC <- function(newformXData = ..., oldformYData = ..., newformCommonItemNames = ..., oldformCommonItemNames = ..., itemtype = '3PL', newformBILOGprior = NULL, oldformBILOGprior = NULL, tryFitwholeNewItems = T, tryFitwholeOldItems = T, ...){
  ...
  if(is.list(oldformYData@Options) && !is.data.frame(oldformYData)){ # if Data is mirt model
    ...
    if(!oldFormModel@OptimInfo$secondordertest){
      ...
      oldFormModel <- surveyFA(oldformYData, autofix = F, SE = T, forceMHRM = T)
    }
  ...
  if(is.list(newformXData@Options) && !is.data.frame(newformXData)){ # if Data is mirt model
    ...
    if(!newFormModel@OptimInfo$secondordertest){
      ...
      newFormModel <- surveyFA(newformXData, autofix = F, SE = T, forceMHRM = T)
    }
  ...
  LinkedModelSyntax <- mirt::mirt.model(paste0('F1 = 1-',ncol(newformXDataK[colnames(newFormModel@Data$data)]),'\n',
                                               'MEAN = F1'))
  LinkedModel <- mirt::mirt(data = newformXDataK[colnames(newFormModel@Data$data)], LinkedModelSyntax, itemtype = newFormModel@Model$itemtype, method = 'MHRM', SE = T, accelerate = 'squarem', TOL = 1e-4, technical = list(NCYCLES = 1e+5, MHRM_SE_draws = 1e+5), pars = NewScaleParms, GenRandomPars = F)
