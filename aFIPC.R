...
autoFIPC <- function(newformXData = ..., oldformYData = ..., newformCommonItemNames = ..., oldformCommonItemNames = ..., itemtype = '3PL', newformBILOGprior = NULL, oldformBILOGprior = NULL, tryFitwholeNewItems = T, tryFitwholeOldItems = T, checkIPD = T, tryEM = F, freeMEAN = T, ...){
  ...
  if(!is.data.frame(oldformYData) && !is.matrix(oldformYData)){ # if Data is mirt model
    ...
    if(!oldFormModel@OptimInfo$secondordertest){
      ...
      oldFormModel <- surveyFA(oldformYData, autofix = F, SE = T, forceUIRT = T, skipS_X2 = skipS_X2, forceMHRM = T)
    }
  ...
  if(!is.data.frame(newformXData) && !is.matrix(newformXData)){ # if Data is mirt model
    ...
    if(!newFormModel@OptimInfo$secondordertest){
      ...
      newFormModel <- surveyFA(newformXData, autofix = F, SE = T, forceUIRT = T, skipS_X2 = skipS_X2, forceMHRM = T)
    }
