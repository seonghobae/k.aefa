...
fastFIFA <- function(x, covdata = NULL, formula = NULL, SE = F, SE.type = "crossprod", skipNominal = T, forceGRSM = F, assumingFake = F, masterThesis = F, forceRasch = F, unstable = F, ...){
  for(i in 1:100){
    ...
    if(i > 1){
      ...
      if(modTEMP@Fit$DIC > modOLD@Fit$DIC | modTEMP@OptimInfo$converged != 1 | sum(rotMat > .999) != 0 | sum(modTEMP@Fit$h2 > .999) != 0){ # modTEMP@Fit$AICc > modOLD@Fit$AICc | 
        message('optimal factor numbers: ', paste0(i-1))
        return(modOLD)
      } else if(sum((colSums(abs(rotMat) > .4) > 3)) != ncol(modTEMP@Fit$F)) {
        message('optimal factor numbers: ', paste0(i-1))
        return(modOLD)
      }
