...
fastFIFA <- function(x, covdata = NULL, formula = NULL, SE = T, SE.type = "Oakes", skipNominal = F,
                     ...
                     diagnosis = F, forceDefalutAccelerater = F, forceDefaultOptimizer = F, ...){
  ...
  for(i in 1:100){
    ...
    if(length(covdata) == 0){ # if no covariate variables
      if(forceMHRM == T | forceGRSM == T | assumingFake == T | masterThesis == T){
        ...
        if (forceNormalEM == T | i == 1){
          estimationMETHOD <- 'EM'
          if(!forceDefalutAccelerater | !forceDefaultOptimizer){
            optimINPUT <- 'nlminb'
          } else {
            optimINPUT <- NULL
          }
    ...
    if(max(x, na.rm = T) - min(x, na.rm = T) == 1){ # dichotomous items
      ...
      if(skipNominal == F){
        if(skipIdealPoint == F){
          ...
          try(modTEMP <- mirt::mirt(data = x, model = i, itemtype = itemtype_mixed, method = estimationMETHOD,
                                    accelerate = accelerateINPUT, calcNull = T,
                                    technical = list(MAXQUAD = 20000000000, MHRM_SE_draws = MHRM_SE_draws, symmetric = symmetricINPUT, SEtol = SEtolINPUT,
      ...
      if(exists('modTEMP') == F && forceNRM == F){
        ...
        try(modTEMP <- mirt::mirt(data = x, model = i, method = estimationMETHOD, itemtype = itemtype_mixed, accelerate = accelerateINPUT,
                                  calcNull = T, technical = list(MAXQUAD = 2000000, delta = 1e-20, MHRM_SE_draws = MHRM_SE_draws, symmetric = symmetricINPUT,
    ...
    if(i == 1 && modTEMP@OptimInfo$converged == F){
      stop('No convergence')
    }
...
surveyFA <- function(data = ..., covdata = NULL, formula = NULL, SE = T,
                     ...
                     pilotTestMode = F, forceConsiderPositiveZh = F, ...) {
  message('---------------------------------------------------------')
  ...
  while (!itemFitDone) {
    ...
    if(ncol(surveyFixModRAW) > 3){
      ...
      if(length(which(surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems] < -1.96)) != 0 && sum(is.na(surveyFixModRAW)) == 0){ # Drasgow, F., Levine, M. V., & Williams, E. A. (1985). Appropriateness measurement with polychotomous item response models and standardized indices. British Journal of Mathematical and Statistical Psychology, 38(1), 67-86.
        message('\nDrasgow, F., Levine, M. V., & Williams, E. A. (1985) / ', surveyFixMod_itemFit$item[which(min(surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems]) == surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems])])
        ...
        if(length(which(surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems] > 1.96)) != 0 && sum(is.na(surveyFixModRAW)) == 0 && forceConsiderPositiveZh){ # Drasgow, F., Levine, M. V., & Williams, E. A. (1985). Appropriateness measurement with polychotomous item response models and standardized indices. British Journal of Mathematical and Statistical Psychology, 38(1), 67-86.
          message('\nDrasgow, F., Levine, M. V., & Williams, E. A. (1985) / ', (surveyFixMod_itemFit$item[which(max(surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems]) == surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems])]))
        ...
        if(sum(is.na(surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems])) == 0 && sum(na.omit(surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems])) == 0){ # avoid unexpected situation
          ...
          message('\nremoving items df is NA / ', (surveyFixMod_itemFit$item[which(is.na(surveyFixMod_itemFit$df.S_X2) == TRUE)]))
          ...
          message('\nremoving items df is 0 / ', (surveyFixMod_itemFit$item[which(surveyFixMod_itemFit$df.S_X2 == 0)]))
          ...
          message('\nremoving items p.S_X2 is NA / ', (surveyFixMod_itemFit$item[which(is.na(surveyFixMod_itemFit$p.S_X2) == TRUE)]))
          ...
          message('\nDrasgow, F., Levine, M. V., Tsien, S., Williams, B., & Mead, A. D. (1995) / ', (surveyFixMod_itemFit$item[which(max(surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems]) == surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems])]))
          ...
          message('\nKang, T., & Chen, T. T. (2008); Reise, S. P. (1990) / ', (surveyFixMod_itemFit$item[which(max(surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems]) == surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems])]))
          workKeys <- workKeys[-which(max(surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems]) == surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems])]
...
fastMultipleGroup <- function(x, covdata = NULL, formula = NULL, SE = F, SE.type = "Oakes", skipNominal = F,
                              forceGRSM = F, assumingFake = F, masterThesis = F, forceRasch = F, unstable = F,
  ...
  for(i in 1:100){
    ...
    if(max(x, na.rm = T) - min(x, na.rm = T) == 1){ # dichotomous items
      ...
      if(nrow(x) >= 5000){
        ...
        if(exists('modTEMP') == F){
          ...
          message('\nMIRT model: Compensatory 3PL with lower or upper asymptote estimated')
      ...
      if(exists('modTEMP') == F | modTEMP@OptimInfo$converged == F){
      ...
      if(nrow(x) >= 5000){
        ...
        if(exists('modTEMP') == F){
          ...
          message('\nMIRT model: Compensatory 3PL Nominal response with lower or upper asymptote estimated')
    ...
    if(i > 1){
      ...
      if(modTEMP@Fit$DIC > modOLD@Fit$DIC | modTEMP@OptimInfo$converged == F | sum(round(rotMat, 2) >= 1.00) != 0 | sum(round(modTEMP@Fit$h2, 2) >= .99) != 0){ # modTEMP@Fit$AICc > modOLD@Fit$AICc | 
        message('optimal factor numbers: ', paste0(i-1))
        return(modOLD)
      } else if(sum(colSums(round(abs(rotMat), 2) > .4) < 2) != 0) {
        message('optimal factor numbers: ', paste0(i-1))
...
deepFAengine <- function(mirtModel, survey.weights){ # for search more factors with prevent local optimal
  ...
  if(bestModel == 1){
    ...
    return(mirt::mirt(data = mirtModel@Data$data, model = nfact[bestModel], itemtype = mirtModel@Model$itemtype, SE = mirtModel@Options$SE, SE.type = mirtModel@Options$SE.type, covdata = attr(mirtModel@ParObjects$lrPars, 'df'), formula = attr(mirtModel@ParObjects$lrPars, 'formula')[[1]], method = method, optimizer = mirtModel@Options$Moptim, accelerate = mirtModel@Options$accelerate, verbose = F, technical = list(NCYCLES = mirtModel@Options$technical$NCYCLES, MAXQUAD = mirtModel@Options$technical$MAXQUAD, SEtol = mirtModel@Options$technical$SEtol, symmetric = mirtModel@Options$technical$symmetric, removeEmptyRows = mirtModel@Options$technical$removeEmptyRows, MHRM_SE_draws = mirtModel@Options$technical$MHRM_SE_draws)))
  }
}
