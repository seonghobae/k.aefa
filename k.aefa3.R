    ...
    if(max(x, na.rm = T) - min(x, na.rm = T) == 1){ # dichotomous items
      ...
      if(forceRasch == T){
        ...
        try(modTEMP <- mirt::mirt(data = x, model = i, itemtype = 'Rasch', method = estimationMETHOD,
                                  ...
                                  technical = list(MHRM_SE_draws = MHRM_SE_draws, symmetric_SEM = symmetric_SEMINPUT, SEtol = SEtolINPUT,
      ...
      if(exists('modTEMP') == F){
        ...
        try(modTEMP <- mirt::mirt(data = x, model = i, method = estimationMETHOD, itemtype = itemtype_mixed, accelerate = accelerateINPUT,
                                  calcNull = T, technical = list(MHRM_SE_draws = MHRM_SE_draws, symmetric_SEM = symmetric_SEMINPUT,
...
surveyFA <- function(data = ..., covdata = NULL, formula = NULL, SE = F,
                     ...
                     forceUIRT = F, skipIdealPoint = F, MHRM_SE_draws = 1e+4, bifactorSolution = F, skipS_X2 = F, ...) {
  message('---------------------------------------------------------')
  ...
  while (!itemFitDone) {
    ...
    if(ncol(surveyFixModRAW) > 3){
      ...
      if(activateInfitOnly == T){ # if can't calculate S-X2 fit when itemtype = 'Rasch'
        ...
        if(length(which(surveyFixMod_itemFit$Zh[1:surveyFixMod@Data$nitems] < -1.96)) != 0){ # Drasgow, F., Levine, M. V., & Williams, E. A. (1985). Appropriateness measurement with polychotomous item response models and standardized indices. British Journal of Mathematical and Statistical Psychology, 38(1), 67-86.
        ...
        } else if(nrow(data) <= 5000 && sum(is.na(surveyFixMod_itemFit$p.S_X2[1:surveyFixMod@Data$nitems])) == 0 && length(which(surveyFixMod_itemFit$S_X2[1:surveyFixMod@Data$nitems]/surveyFixMod_itemFit$df.S_X2[1:surveyFixMod@Data$nitems] >= 3)) != 0 && skipS_X2 == F){ # Drasgow, F., Levine, M. V., Tsien, S., Williams, B., & Mead, A. D. (1995). Fitting polytomous item response theory models to multiple-choice tests. Applied Psychological Measurement, 19(2), 143-166.
...
bifactorFA <- function(data = ..., skipS_X2 = F, forceMHRM = F, covdata = NULL, formula = NULL) {
  mod <- surveyFA(data = data, bifactorSolution = T, skipS_X2 = skipS_X2, forceMHRM = forceMHRM, autofix = F, covdata = covdata, formula = formula)
  STOP <- FALSE
  while (!STOP) {
    if(ncol(mod@Fit$F) == 1){
    ...
    if(sum(rotMAT < .99) != ncol(mod@Data$data)){
      mod <- surveyFA(data = mod@Data$data[,-which(rotMAT == max(rotMAT))], bifactorSolution = T, skipS_X2 = skipS_X2, forceMHRM = forceMHRM, autofix = F, covdata = covdata, formula = formula)
    } else if(sum(rotMAT > .1) != ncol(mod@Data$data)){
      mod <- surveyFA(data = mod@Data$data[,-which(rotMAT == min(rotMAT))], bifactorSolution = T, skipS_X2 = skipS_X2, forceMHRM = forceMHRM, autofix = F, covdata = covdata, formula = formula)
    } else {
      return(mod)
    }
